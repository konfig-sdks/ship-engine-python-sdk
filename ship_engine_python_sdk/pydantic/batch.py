...
from ship_engine_python_sdk.pydantic.batch_status import BatchStatus
from ship_engine_python_sdk.pydantic.date_time import DateTime
from ship_engine_python_sdk.pydantic.date_time_nullable import DateTimeNullable
from ship_engine_python_sdk.pydantic.error import Error
...
class Batch(BaseModel):
    # label layout
    label_layout: LabelLayout = Field(alias='label_layout')

    label_format: LabelFormat = Field(alias='label_format')

    # A string that uniquely identifies the batch
    batch_id: SeId = Field(alias='batch_id')

    # The batch number.
    batch_number: str = Field(alias='batch_number')

    # A string that uniquely identifies the external batch
    external_batch_id: typing.Optional[str] = Field(alias='external_batch_id')

    # Custom notes you can add for each created batch
    batch_notes: typing.Optional[str] = Field(alias='batch_notes')

    # The date and time the batch was created in ShipEngine
    created_at: DateTime = Field(alias='created_at')

    # The date and time the batch was processed in ShipEngine
    processed_at: DateTimeNullable = Field(alias='processed_at')

    # The number of errors that occurred while generating the batch
    errors: int = Field(alias='errors')

    # The errors associated with the failed API call
    process_errors: typing.List[Error] = Field(alias='process_errors')

    # The number of warnings that occurred while generating the batch
    warnings: int = Field(alias='warnings')
    ...
    forms: int = Field(alias='forms')

    # The total of errors, warnings, and completed properties
    count: int = Field(alias='count')

    # The batch shipments endpoint
    batch_shipments_url: OptionalLink = Field(alias='batch_shipments_url')

    # Link to batch labels query
    batch_labels_url: OptionalLink = Field(alias='batch_labels_url')

    # Link to batch errors endpoint
    batch_errors_url: OptionalLink = Field(alias='batch_errors_url')

    # The label download for the batch
    label_download: LabelDownload = Field(alias='label_download')

    # The form download for any customs that are needed
    form_download: OptionalLink = Field(alias='form_download')

    status: BatchStatus = Field(alias='status')
